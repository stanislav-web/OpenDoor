...
class TestReleaseCoverageGaps(unittest.TestCase):
    ...
    def test_release_e2e_command_keeps_crawl_smoke_enabled(self):
        """release.sh should smoke-test --crawl during local e2e verification."""

        release_script = Path('release.sh').read_text(encoding='utf-8')

        self.assertIn('--scan directories', release_script)
        self.assertIn('--crawl', release_script)
        self.assertLess(
            release_script.index('--scan directories'),
            release_script.index('--crawl'),
        )
        self.assertLess(
            release_script.index('--crawl'),
            release_script.index('--reports json,sarif'),
        )
