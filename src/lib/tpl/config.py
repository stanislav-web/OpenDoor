...
class Config:
    ...
    templates = {
        ...
        'browser': 'Fetching user-agent: {browser}', 'directories': 'Read {total} directories list by line',
        'create_queue': 'Creating queue with {threads} thread(s)...',
        'subdomains': 'Read {total} subdomains list by line...',
        'random_browser': 'Fetching random user-agent per request...', 'get_item_lvl0': '{percent} {item}',
        ...
        'resume_threads': 'Resuming scan...', 'get_item_lvl1': '{percent} [{current}/{total}] - {size} - {item}',
        'addtopool': 'Adding {total} lines to queue...', 'http_pool_start': 'Using HTTP keep-alive connection',
        ...
        'proxy_pool_external_start': 'Fetching external proxy list...', 'request_header_dbg': 'Request header:\n{dbg}',
        'response_header_dbg': 'Response header:\n{dbg}', 'ignored_path': 'Ignored. The path {path} in ignore list',
        ...
        'read_timeout_error': 'Connection timeout! {url}. Increase using --timeout option', 'success': 'OK {url}',
        'file': 'File {url}', 'indexof': 'Index {url}', 'forbidden': 'Denied {url}', 'redirect': 'R {url} -> {rurl}'}
